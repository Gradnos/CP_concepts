...
vector<Point> hp_intersect(vector<Halfplane>& H) { 
    ...
    for(int i = 0; i < int(H.size()); i++) {
        // Remove from the back and front of the deque while last half-plane is redundant
        while (len > 1 && H[i].out(inter(dq[len-1], dq[len-2]))) {dq.pop_back();--len;}
        while (len > 1 && H[i].out(inter(dq[0], dq[1]))) {dq.pop_front();--len;}
        if (len > 0 && fabsl(cross(H[i].pq, dq[len-1].pq)) < eps) { // this check is needed if we have parallel planes
    ...
    if (len < 3) return vector<Point>(); // no intersection
    vector<Point> ret(len); // reconstruct the points(may have duplicates if 3 lines cross on a point)
